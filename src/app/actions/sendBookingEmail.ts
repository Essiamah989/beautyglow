...
export async function sendBookingEmail(data: {
  ...
  try {
    ...
    await resend.emails.send({
      from: "BeautyGlow <onboarding@resend.dev>", // ← use this for now
      to: 'mahmoudiessia989@gmail.com',  // ← hardcode your email for testing
        ...
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; background: #0d0d0d; color: #e8e0d5; padding: 40px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              ...
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a;">${data.customer_name}</td>
            </tr>
            <tr>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a; color: #666; font-size: 0.8rem;">Téléphone</td>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a;">${data.customer_phone}</td>
            </tr>
            <tr>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a; color: #666; font-size: 0.8rem;">Service</td>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a;">${service?.name || "—"}</td>
            </tr>
            <tr>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a; color: #666; font-size: 0.8rem;">Date</td>
              <td style="padding: 12px 0; border-bottom: 1px solid #1a1a1a;">${data.booking_date}</td>
            </tr>
            <tr>
              <td style="padding: 12px 0; color: #666; font-size: 0.8rem;">Heure</td>
              <td style="padding: 12px 0;">${data.booking_time}</td>
            </tr>
          </table>
          <div style="margin-top: 32px; padding: 20px; background: rgba(201,169,110,0.08); border-left: 2px solid #c9a96e;">
            <p style="color: #c9a96e; font-size: 0.8rem; margin: 0;">Connectez-vous à votre dashboard pour confirmer.</p>
          </div>
